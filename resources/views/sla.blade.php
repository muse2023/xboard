...
<meta property="og:title" content="服务水平保障协议 - 高速网络,企业服务 &amp; 匿名安全 高可用性服务商 | {{$title}}">
...
<li class="nav-item">
<a class="nav-link active" href="#section-1">1. 定义</a>
</li>
<li class="nav-item">
<a class="nav-link" href="#section-2">2. 丢包与延迟</a>
</li>
<li class="nav-item">
<a class="nav-link" href="#section-3">3. SLA 排除</a>
</li>
<li class="nav-item">
<a class="nav-link" href="#section-4">4. 最高金额</a>
</li>
<li class="nav-item">
<a class="nav-link" href="#section-4">5. 获取信用额度</a>
</li>
</ul>
</div>
...
<p>在执行 {{$title}} 许可协议（适用于“协议”）的期限内，涵盖服务将提供客户的每月正常运行时间百分比至少为 90%（“服务级目标”或“SLO”）。如果 {{$title}} 不符合 SLO 规定，且客户履行了本 SLA 规定的义务，客户将有资格获得下述中断补偿。该 SLA 声明客户唯一和补救措施唯一，如果 {{$title}} 不符合 SLO 规定。本 SLA 中使用的但不在本 SLA 中定义的术语具有协议中阐述的含义。</p>
</div>
<div id="section-1" class="mb-7">
<h4>定义</h4>
...
<li>“<strong>中断时长</strong>”是指：由于 {{$title}} 的系统故障，在同一区域的两个或多个区域中正确配置的所有接入点无法提供指向其接入点的任何流量，以及无法切换到该区域中其它接入点。</li>
...
<div id="section-2" class="mb-7">
<h4>丢包与延迟</h4>
<p>{{$title}} 会主动监测各项服务的运行状况，在运营商出现网络抖动时会及时调整路由策略，并尽可能的减少由于网络抖动所带来的影响。但客户在实际使用中，仍然会受到客户的运营商路由策略、运营商网络调度、线缆信号、终端质量等多因素影响。</p>
<p>由于上述原因，所以 {{$title}} 不将延迟作为 SLA 的保障。</p>
</div>
<div id="section-3" class="mb-7">
<h4>中断补偿</h4>
...
<div id="section-5" class="mb-7">
<h4>最高金额</h4>
<p>在单个结算月份内发生的任何和所有中断期间，{{$title}} 向客户发放的最高总信用额不得超过客户在适用月份的涵盖服务中应付的金额的 50%，信用额度将以账户余额的方式发放，并将在申请补偿后的 7 天内进行。</p>
</div>
<div id="section-6" class="mb-7">
<h4>获取信用额度</h4>
<p>为了获取服务中断所提供的 SLA 补偿，客户应当提交一个工单到 {{$title}}，并明确要求获取相应的补偿。</p>
<p>客户发送工单请求补偿时，需要提供所请求补偿的产品编号，并且在该工单中描述服务中断的问题和发生时间，中断时长（请参照上文）。</p>
<p>只有购买 {{$title}} 书面说明受 SLA 保证的服务，才能向 {{$title}} 请求信用额度补偿。如果客户不符合这些要求，客户将丧失获得信用额度补偿的权利。如果针对此 SLA 发生争议，{{$title}} 将根据其系统日志，监控报告，配置记录和其他可用信息进行真诚的决定，{{$title}} 将根据客户的要求向客户提供审核。</p>
</div>
